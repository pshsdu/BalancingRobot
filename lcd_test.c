...
void calAngTarget(void){
        ...
        sumBodyVelErr += bodyVelErr/50; //i
        
        if(sumBodyVelErr > 1000)          sumBodyVelErr = 1000;
        else if(sumBodyVelErr < -1000) sumBodyVelErr = -1000;
...
interrupt [TIM0_COMPA] void timer0_compa_isr(void)
{
        ...
        //mDistance +=(RVel+LVel)/2 ;
        angVel = (rollAngle * 76) >> 4;
        centerVel = ((LVel + RVel)  * 67) >> 2;
        
        mBodyVel = centerVel + angVel;
        filteredBodyVel = (int)(filteredBodyVel*0.99 + mBodyVel*0.01);  //0.1% 만큼 반영
